...
import { History, Home, LogOut, Scale, Sparkles, UserRound, ChevronRight, Activity, AlertCircle, ShieldAlert, Network, ChevronDown, Database, Cpu, BrainCircuit, Settings } from "lucide-react";
...
export default function MainPortal({ user, onLogout }) {
  ...
  // Fetch history from the backend API (uses service-role client, bypasses RLS)
  const refreshHistory = async () => {
    try {
      const { data: sessionData } = await supabase.auth.getSession();
      const token = sessionData?.session?.access_token;
      if (!token) return;
      const res = await fetch("http://127.0.0.1:8000/api/history?page=1&page_size=50", {
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (!res.ok) throw new Error(`History fetch failed: ${res.status}`);
      const json = await res.json();
      const rows = (json.items || []).map(d => ({
        id: d.id,
  ...
  const handlePredict = async (event) => {
    ...
    const parsed = {
      age: parseNumericInput(castingInputs.age), sex: castingInputs.sex ? castingInputs.sex.trim().toUpperCase() : "",
    ...
    const missingKeys = Object.entries(parsed).filter(([key, value]) => key !== "sex" && value === null).map(([key]) => key);
    ...
    try {
      ...
      let prediction;
      if (token) {
        // Use the actual backend API which also manages DB insert properly
        ...
        if (!res.ok) {
           ...
           console.error("API Prediction Error:", errMsg);
      ...
      // Refresh history from DB so the new record shows up in the history tab
      await refreshHistory();
      
    } catch(err) {
      console.error("Prediction fell back to client simulation due to API error:", err);
      
      const fallbackPred = getFallbackPrediction(parsed);
      const entry = { id: `${Date.now()}-${Math.random()}`, ...parsed, ...fallbackPred.summary, source: fallbackPred.source, createdAt: new Date().toISOString() };
      ...
      if (supabase && user?.id) {
         try {
            ...
            if (dbError) {
              console.error("Fallback DB Insert Error:", dbError);
              setFormError(`DB Storage Error: ${dbError.message}`);
            }
         } catch(e) {
              ...
              <div className="relative w-full rounded-[2.5rem] p-10 md:p-[4.5rem] overflow-hidden border border-white/10 group shadow-[0_0_120px_rgba(34,211,238,0.07)] bg-black">
                ...
                <div className="relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto">
                  ...
                  <h2 className="font-serif text-6xl md:text-8xl tracking-[0.1em] font-light text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.4)] mb-6 leading-tight uppercase flex justify-center items-center gap-3">
                    CASE <span className="font-serif italic font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-400 to-purple-500 drop-shadow-[0_0_40px_rgba(34,211,238,0.5)]">CAST</span>
                  </h2>
                  <p className="text-slate-400 text-lg md:text-xl leading-relaxed mb-10 max-w-2xl font-light">
                    A highly advanced system that analyzes historical data and population trends to produce clear, useful insights                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mt-4">
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, ease: "easeOut" }} className="group relative p-[1px] rounded-3xl overflow-hidden bg-white/5 hover:bg-white/10 transition-transform hover:-translate-y-1 hover:scale-[1.02] duration-300 cursor-default">
                  ...
                  <div className="relative h-full bg-black border border-transparent group-hover:border-white/5 backdrop-blur-3xl rounded-[23px] p-8 flex flex-col gap-5 transition-colors z-10 w-[calc(100%-2px)] m-[1px]">
                    ...
                    <p className="text-slate-400 text-sm leading-relaxed">
                      Naturally predicts possible court decisions and how likely different verdicts are.<br />Heavily influenced by prior offense records and historical precedence matrices.
                    </p>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }} className="group relative p-[1px] rounded-3xl overflow-hidden bg-white/5 hover:bg-white/10 transition-transform hover:-translate-y-1 hover:scale-[1.02] duration-300 cursor-default">
                  ...
                  <div className="relative h-full bg-black border border-transparent group-hover:border-white/5 backdrop-blur-3xl rounded-[23px] p-8 flex flex-col gap-5 transition-colors z-10 w-[calc(100%-2px)] m-[1px]">
                    ...
                    <p className="text-slate-400 text-sm leading-relaxed">
                      Classify active legal infractions completely automatically.<br />Easily identifies which offenses are bailable and which are non-bailable under the law.
                    </p>
                  </div>
                </motion.div>
              ...
              <div className="w-full xl:w-5/12 flex flex-col gap-4 order-2 xl:order-1 h-auto relative z-10 mb-12">
                <div className="glass-card p-5 md:p-6 flex flex-col gap-5">
                  <div className="flex justify-between items-center mb-1">
                    <h2 className="font-display text-xl font-bold text-white flex items-center gap-2"><Sparkles className="w-5 h-5 text-cyan-400" /> Prediction Inputs</h2>
                  </div>

                  <form id="predictor-form-container" onSubmit={handlePredict} className="flex flex-col gap-5">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <label className="flex flex-col gap-1.5"><span className="text-[13px] uppercase font-bold text-slate-400 tracking-wider">Age (18+)</span><input name="age" type="number" min="18" max="120" className="input-glass bg-slate-900/60 py-2 px-3 text-sm" placeholder="e.g. 35" value={castingInputs.age} onChange={e => updateCastingInput('age', e.target.value)} /></label>
                      <label className="flex flex-col gap-1.5"><span className="text-[13px] uppercase font-bold text-slate-400 tracking-wider">Sex</span><select name="sex" className="input-glass bg-slate-900/60 py-2 px-2 text-sm" value={castingInputs.sex} onChange={e => updateCastingInput('sex', e.target.value)}><option value="">Sel</option><option value="M">M</option><option value="F">F</option></select></label>
                      <label className="flex flex-col gap-1.5"><span className="text-[13px] uppercase font-bold text-slate-400 tracking-wider">Priors</span><input name="priorOffenses" type="number" min="0" className="input-glass bg-slate-900/60 py-2 px-3 text-sm" placeholder="0" value={castingInputs.priorOffenses} onChange={e => updateCastingInput('priorOffenses', e.target.value)} /></label>
                      <label className="flex flex-col gap-1.5"><span className="text-[13px] uppercase font-bold text-slate-400 tracking-wider">Jail Days</span><input name="jailDurationDays" type="number" min="0" className="input-glass bg-slate-900/60 py-2 px-3 text-sm" placeholder="0" value={castingInputs.jailDurationDays} onChange={e => updateCastingInput('jailDurationDays', e.target.value)} /></label>
                    </div>
              ...
              <div className="w-full xl:w-7/12 flex flex-col order-1 xl:order-2 self-stretch min-h-[600px] justify-center relative mt-6 xl:mt-0 pb-12">
                ...
                {result && !isPredicting && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col gap-6 h-full">
                    ...
                    <div className={`p-10 rounded-3xl border shadow-2xl relative overflow-hidden backdrop-blur-xl ${result.summary.outcome.includes("High") ? 'bg-rose-950/20 border-rose-500/50 shadow-rose-500/20' :
